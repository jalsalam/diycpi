...
# CPI
cpi <- read_tsv("cu.data.2.Summaries") %>%
...
# Breakdown choices
cx_demographics <- read_tsv("cx.demographics") %>%
...
cx_characteristics <- read_tsv("cx.characteristics") %>%
...
# the data
cx_data_1_AllData <- read_tsv("cx.data.1.AllData") %>%
   ...
   sidebarLayout(
      sidebarPanel(
        ...
        selectInput("demographics_code",
         ...
         checkboxGroupInput("characteristics_codes",
                     label = "Select specific characteristics:",
                     choices = initial_characteristics_choices,
                     selected = c("All Consumer Units" = "01") # , multiple = TRUE
                    )
      ),
...
server <- function(input, output, clientData, session) {
  ...
  observeEvent(input$demographics_code, {
    ...
    updateCheckboxGroupInput(session, "characteristics_codes", 
  ...
  data <- reactive({
    ...
    scale_x_continuous(breaks = (c(1984, 1994, 2004, 2016))) +
    theme_bw() +
